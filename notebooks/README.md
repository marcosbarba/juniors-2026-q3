# EDA — STARWARS_AUTOCALLS

Resumen del análisis exploratorio de las tres tablas fuente (`rfqs.csv`, `daily_volatility.csv`, `underlyings_reference.csv`) antes de abordar la integración y el modelado. Recoge las hipótesis de negocio de partida, las decisiones tomadas durante el preprocesamiento, los hallazgos de cada tabla y las limitaciones conocidas del dataset.

Notebooks de referencia: `rfqs_EDA.ipynb`, `daily_volatility_EDA.ipynb`, `underlyings_reference_EDA.ipynb`.
...
- **`observation_frequency`** normalizada de 18 categorías (duplicadas por idioma/formato: `1M`, `Monthly`, `mensual`...) a 6 categóricas limpias (`Monthly`, `Bimonthly`, `Quarterly`, `Semiannual`, `Annual`, `Daily`). Pendiente de decidir codificación final (ordinal por "meses entre observaciones" es viable con modelos de árboles, pese al salto de escala hacia `Daily`).
...
- `nominal_term_months` correlaciona positivamente con la duración en todos los productos (actúa como techo físico), con `Wretched Hive Digital` destacando en 0.898 (ver perfil). *Hallazgo documentado a nivel de EDA:* finalmente no se usa ocmo feature de modelado (ver Decisones de preprocesamiento).
...
- El signo invertido de `quoted_implied_vol` según familia de producto queda documentado pero no explicado causalmente con las columnas disponibles.
...
## Pendiente para la fase de integración

- Diseño del join punto-en-el-tiempo entre `rfqs` (a `requested_date`) y `daily_volatility` (formato ancho), con ventana de 63 días hacia atrás para nivel de volatilidad, tendencia y correlación de cesta (media y mínimo de correlaciones por pares; caso especial cestas `single` → 1).
- Agregación de `structural_base_vol` por cesta (media y desviación, con desviación = 0 para `single`).
- Codificación final de `observation_frequency_clean` (ordinal vs. one-hot) y de `counterparty`/`trader_id` (K-fold target encoding).
