...
- Solo la información disponible en el momento `requested_date` es válida como input: es el único momento en el que la mesa cotiza sin conocer el desenlace.

## Decisiones de preprocesamiento

- **`executed`, `start_date`, `end_date` no se usan directamente como features**, pero se investigó si contradecían la premisa del enunciado (ver más abajo).
...
- El ratio `vol_media_realizada / structural_base_vol` es consistente entre 1.20–1.24 para 12 de los 14 subyacentes, salvo `HTTX` (1.75) y sobre todo `REBL` (2.65) (ver perfiles atípicos debajo).
...
- `realized_vol_63d` es la única serie temporal por subyacente disponible; su autocorrelación de lag-1 (>0.97 en todos los casos) infla cualquier correlación de cesta calculada directamente sobre ella. Esto muestra una limitación estructural del dataset entregado, no evitable sin datos de retornos diarios brutos.
...
- **Métrica** elegida en función de la forma del target (ver Hallazgos `rfqs.csv`: fuerte asimetría a la derecha, cola hasta 127 meses, mínimo cercano a cero, 0.16):
  ...
  - **RMSE/MSE descartados como métrica única (aunque sí se reportan como secundaria)**: al elevar al cuadrado, sobreponderan los errores de la cola larga del target frente al grueso de observaciones, una métrica principal basada solo en RMSE optimizaría el modelo hacia minimizar errores en los casos extremos a costa de la precisión en el rango típico (25-50 meses, donde se concentra la mayoría de las RFQs).
  - **MAE elegida como métrica principal**: robusta frente a la cola larga (no eleva al cuadrado, cada error pesa proporcionalmente a su magnitud), directamente interpretable en meses (una unidad que tiene significado de negocio inmediato), y no distorsionada por los valores cercanos a cero que invalidan el MAPE.
...
- 5 algoritmos comparados con `TimeSeriesSplit` (5 folds) **dentro de train únicamente**, test se mantuvo intacto hasta la evaluación final, para no contaminar la decisión de qué modelo elegir: RandomForest y GradientBoosting (one-hot + K-fold target encoding para `category_encoders.TargetEncoder`), HistGradientBoosting, XGBoost y LightGBM (categóricas nativas, sin one-hot, K-fold target encoding para `category_encoders.TargetEncoder`).
- Con hiperparámetros por defecto, XGBoost salía claramente peor (MAE 12.21 frente a 11.57–11.70 del resto), identificado como consecuencia de un `learning_rate` por defecto distinto (0.3 frente a 0.1 en los demás), no de una diferencia real de capacidad del algoritmo. Al igualarlo a 0.1, XGBoost sube a 11.56.
...
- **Predicho vs. real (test)**: R² 0.534. El modelo ajusta razonablemente bien en el rango típico (0-60 meses, donde se concentra la mayoría de las RFQs), pero infraestima sistemáticamente las duraciones largas (>70 meses), las predicciones se aplanan en torno a 60-80 incluso cuando el valor real llega a 120. Es coherente con la ausencia de `nominal_term_months` como feature (excluida por fidelidad al contrato de datos, ver Decisiones de preprocesamiento): esa variable actúa como techo físico de la duración y era, precisamente, el predictor más fuerte de los casos de cola larga en la EDA (correlación 0.53 global). Sin ella, el modelo carece de la señal más directa para anticipar que una RFQ concreta tiene margen contractual para durar mucho, y por eso su punto más débil coincide exactamente con el rango donde esa variable habría aportado más.
